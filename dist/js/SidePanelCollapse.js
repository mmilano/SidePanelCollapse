...
(function (window, SidePanelCollapse) {
  ...
  var data_selector = "[data-sidepanel-collapse]";
  /**
  ...
  function defineSettings(defaults, options) {
    ...
    var _settings = Object.assign({}, defaults); // reconcile with any provided options that will supercede/overwrite defaults


    Object.assign(_settings, options); // if backdrop is anything other than "true", it is false

    _settings.backdrop = _settings.backdrop === true ? true : false; // create a flag for the durationShow setting because it is a special case.
  ...
  var defaults = {
    ...
    // top-level of the sidepanel
    sidepanelCloseElement: ".sidePanel-close",
    // the close button, containing the close icon, visible when the sidepanel is displayed
  ...
  function linkHandle() {
    ...
    return function (e) {
      e.preventDefault();
      this.closeType = "fast"; // create link event handler with closure
  ...
  _proto.open = function (e) {
    ...
    function whenTransitionEnds(_this) {
      var handler = function handler() {}; //var handler = function(e) {
      // no action currently
      //};
    ...
    document.addEventListener("keyup", this.handleKey); // jquery event listener to run ONCE on the Bootstrap "is shown" event

    this.$sidepanel.one("shown.bs.collapse", whenTransitionEnds(this)); // add a class to the document's <body>.
  ...
  _proto.close = function (e) {
    ...
    var whenTransitionEnds = function whenTransitionEnds(e) {
    ...
    this.closeQueued = false; // if open is invoked via event (e.g. click the button), then event will exist.
    ...
    this.$sidepanel.one("hidden.bs.collapse", whenTransitionEnds); // manually set the duration so that closing transition uses a custom duration.
    ...
    this.$sidepanel[0].style.transitionDuration = _duration; // initiate the hiding

    this.hide(); // initiate the hiding of backdrop if truthy
  ...
  Backdrop.prototype.hide = function () {
     ...
     * @param {*} e event
     */
    var whenAnimationEnds = function whenAnimationEnds(e) {
      e.target.classList.remove("show"); // note: if eventlistener {once: true} is not available (browser support), then eventListener should be removed manually, e.g.:
    ...
    this.element.addEventListener("animationend", whenAnimationEnds, {
      once: true,
      passive: true
  ...
  function Backdrop(_sidepanel) {
    ...
    var create = function create(style) {
  ...
  function SidePanelCollapse(options) {
    ...
    var _$sidepanel = this.$sidepanel = $(_settings.sidepanelElement); // check if sidepanel exists on the page;
    // if not, exit early.
    ...
    this.handleKey = this.handleKey.bind(this); // durationShow is a special case, because it is the one duration value that is set initially in the css.
    ...
    this.$sidepanel.on("show.bs.collapse", this.open); // (try to) select and cache the close button element.
    ...
    if (_settings.handleLinks) {
      var sidepanelLinks = this.$sidepanel[0].getElementsByTagName("a"),
  ...
  function initOnData() {
    window.SidePanel = [];
    var list = document.querySelectorAll(data_selector);
...
//# sourceMappingURL=map/SidePanelCollapse.js.map
